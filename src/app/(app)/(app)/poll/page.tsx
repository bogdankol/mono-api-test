  ...
  return (
    <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{ 
        padding: '10px', 
        marginBottom: '20px', 
        backgroundColor: status === 'polling' ? '#e3f2fd' : status === 'error' ? '#ffebee' : '#f5f5f5',
        borderRadius: '4px'
      }}>
        Status: {status === 'polling' ? '🔄 Polling...' : status === 'error' ? '❌ Error' : '✅ Connected'}
      </div>
      ...
      <div>
        ...
        {notifications.length === 0 && (
          <p style={{ color: '#999' }}>No notifications yet. Send one to see it appear!</p>
        )}
      </div>
    </div>
  );
